  ...
  <style>
      ...
      .insight-box { background-color: #F0F9FF; border-left: 4px solid #0EA5E9; padding: 1rem; margin: 1rem 0; border-
  radius: 0 8px 8px 0; color: #1E40AF; }
      .warning-box { background-color: #FEF3C7; border-left: 4px solid #F59E0B; padding: 1rem; margin: 1rem 0; border-
  radius: 0 8px 8px 0; color: #92400E; }
      .success-box { background-color: #D1FAE5; border-left: 4px solid #10B981; padding: 1rem; margin: 1rem 0; border-
  radius: 0 8px 8px 0; color: #065F46; }
  </style>
  ...
  def generate_dynamic_analysis(brand_data, brand_name):
      ...
      return f"{icon} **{brand_name}**：份额从 {start_share:.1f}%（{start_year}）→ {end_share:.1f}%（{end_year}），
  {direction}{abs(share_change):.1f}个百分点（{'+' if pct_change > 0 else ''}{pct_change:.1f}%）"
  ...
  st.markdown('<p class="sub-header">深度分析乔丹品牌及国产/国际品牌在马拉松赛场上的地位变化</p>',
  unsafe_allow_html=True)
  ...
  with tab2:
      ...
      if len(jordan_data) == 0:
          ...
          # 计算更详细的分析
          yearly_jordan = jordan_data.groupby('year').agg({'share_pct': 'mean', 'rank':
  'mean'}).reset_index().sort_values('year')
          if len(yearly_jordan) >= 2:
              ...
              <strong>市场定位：</strong>{"乔丹在全局跑者中保持较稳定的大众化地位，但在破3精英选手中的渗透率持续下滑，表
  明其在高端竞技领域的竞争力正被其他国产品牌蚕食。" if cohort_filter == "破3选手" else "乔丹在全局跑者市场中保持稳定份
  额，品牌认知度较高，但面临来自特步等头部国产品牌的激烈竞争。"}
              </div>
              """, unsafe_allow_html=True)
  ...
  with tab4:
      ...
      if len(type_trend) > 0:
          ...
          dom_first = type_trend[(type_trend['year'] == min_yr) & (type_trend['brand_type'] == 'domestic')]
  ['share_pct'].sum()
          dom_last = type_trend[(type_trend['year'] == max_yr) & (type_trend['brand_type'] == 'domestic')]
  ['share_pct'].sum()
          ...
          with col_r:
              ...
              top10_by_type = filtered_df[filtered_df['rank'] <= 10].groupby(['year',
  'type_zh']).size().reset_index(name='count')
              ...
              if len(top10_by_type) > 0:
                  ...
                  fig.update_layout(height=400, barmode='group', yaxis=dict(title='品牌数量'), xaxis=dict(title='年份',
  dtick=1))
                  st.plotly_chart(fig, use_container_width=True)
          ...
          with cl:
              ...
              dom_trend = filtered_df[filtered_df['brand'].isin(dom_brands)].groupby(['year', 'brand'])
  ['rank'].mean().reset_index()
              ...
              if len(dom_trend) > 0:
                  ...
                  fig.update_layout(height=350, yaxis=dict(autorange='reversed', title='排名'), xaxis=dict(title='年份',
  dtick=1))
                  st.plotly_chart(fig, use_container_width=True)
          ...
          with cr:
              ...
              int_trend = filtered_df[filtered_df['brand'].isin(int_brands)].groupby(['year', 'brand'])
  ['rank'].mean().reset_index()
  ...
  dtick=1))
          ...
          国产品牌在{"破3精英选手" if cohort_filter == "破3选手" else "全局跑者"}群体中{"占据绝对主导地位" if dom_last >
  70 else "正在快速崛起"}，
  ...
  # ==================== 页脚 ====================
  st.markdown("---")
  st.markdown('<div style="text-align:center;color:#64748B;padding:1rem;">📊 马拉松跑鞋品牌分析平台 v2.0 | 数据来源：悦
  跑圈等平台</div>', unsafe_allow_html=True)

  另外，requirements.txt 也要改成这三行，否则 Streamlit Cloud 后续重建环境时还可能出问题：

  streamlit>=1.28.0
  pandas>=2.0.0
  plotly>=5.18.0

  如果你要，我下一条可以继续直接给你“GitHub 网页端一步一步怎么粘贴、提交、触发 Streamlit 重部署”的最短操作说明。


› 这些代码提交之后显示：Script execution error
  File "/mount/src/marathon-shoe-analysis-for-streamlit-by-claude/app.py", line 2
      """
     ^
...
• 这是纯粹的格式问题，不是业务逻辑问题。
  ...
  IndentationError: unexpected indent 出现在第 2 行，说明你贴进去的 app.py 文件开头有多余的空格或 Tab。app.py 顶层代码必
  须从第 1 列开始，尤其是这几行前面不能有任何缩进：
  ...
  1. 在 GitHub 打开 app.py
  2. 点编辑
  3. Ctrl+A 全选
  4. 整个删空
  5. 从我下面这个代码块里重新完整复制
  6. 确保第一行 # -*- coding: utf-8 -*- 顶格，前面没有空格
  7. 提交

  先给你一个最小可验证版本。你先把这个完整粘进去测试；如果能跑，再继续换成完整版：
  ...
  st.set_page_config(page_title="马拉松跑鞋品牌分析", page_icon="🏃", layout="wide")
  ...
  def load_data():
      ...
      df["year"] = df["year"].astype(int)
      df["rank"] = df["rank"].astype(int)
      df["type_zh"] = df["brand_type"].map({
          "domestic": "国产",
          "international": "国际",
          "other": "其他"
      })
      return df, data.get("brands", {})
  ...
  st.title("🏃 马拉松跑鞋品牌分析平台")

  all_events = sorted(df["event"].unique().tolist())
  selected_events = st.sidebar.multiselect("选择赛事", all_events, default=all_events)

  min_year, max_year = int(df["year"].min()), int(df["year"].max())
  year_range = st.sidebar.slider("年份范围", min_year, max_year, (min_year, max_year))
  cohort_filter = st.sidebar.radio("跑者队列", ["破3选手", "全局跑者"], index=0)
  aggregate_mode = st.sidebar.checkbox("聚合所有赛事（取平均）", value=True)
  ...
  st.subheader("品牌对比 - 排名趋势验证")

  selected_brands = st.multiselect(
      "选择品牌",
      sorted(filtered_df["brand"].unique().tolist()),
      default=sorted(filtered_df["brand"].unique().tolist())[:3]
  )
  ...
  if len(selected_brands) >= 2:
      ...
      fig.update_layout(
          height=500,
          yaxis=dict(autorange="reversed", title="排名（越小越好）"),
          xaxis=dict(title="年份", dtick=1)
      )
      st.plotly_chart(fig, use_container_width=True)
  else:
  ...
  如果这份最小版能跑，说明你的问题就是“复制时格式坏了”，不是代码本身。

  还有一件事也要一起改，requirements.txt 必须是这三行纯文本：

  streamlit>=1.28.0
  pandas>=2.0.0
  plotly>=5.18.0

  如果你愿意，我下一条直接给你一份“去掉所有可能复制歧义”的完整版 app.py，我会分段发，避免 GitHub 粘贴时再次出格式问题。


› 直接给我一份app.py文件，让我上传进去


⚠ Heads up, you have less than 10% of your 5h limit left. Run /status for a breakdown.

• 把下面内容完整保存为 app.py 后上传即可。第一行前面不要有空格或 Tab。
  ...
  def load_data():
      ...
      df["rank"] = df["rank"].astype(int)
      df["type_zh"] = df["brand_type"].map(
          {"domestic": "国产", "international": "国际", "other": "其他"}
      )
      return df, data.get("brands", {})
  ...
  def generate_dynamic_analysis(brand_data, brand_name):
      ...
      yearly = (
          brand_data.groupby("year")
          .agg({"share_pct": "mean", "rank": "mean"})
          .reset_index()
          .sort_values("year")
      )
      start_share = yearly.iloc[0]["share_pct"]
      ...
      return (
          f"{icon} **{brand_name}**：份额从 {start_share:.1f}%（{start_year}）→ "
          f"{end_share:.1f}%（{end_year}），{direction}{abs(share_change):.1f}个百分点"
  ...
  def calculate_yearly_rank(data, aggregate=True):
      ...
      if aggregate:
          yearly = (
              data.groupby(["year", "brand", "type_zh"])["share"]
              .mean()
              .reset_index()
          )
          yearly["rank"] = (
              yearly.groupby("year")["share"]
              .rank(ascending=False, method="min")
              .astype(int)
          )
      else:
          yearly = (
              data.groupby(["year", "event", "brand", "type_zh"])["share"]
              .mean()
              .reset_index()
          )
          yearly["rank"] = (
              yearly.groupby(["year", "event"])["share"]
              .rank(ascending=False, method="min")
              .astype(int)
          )
  ...
  with st.sidebar:
      ...
      st.markdown(
          f"### 📅 数据范围\n- 赛事: {len(selected_events)} 场\n"
          f"- 年份: {year_range[0]}-{year_range[1]}\n- 队列: {cohort_filter}"
      )
  ...
  # ==================== 主页面 - Tab布局 ====================
  st.markdown(
      '<p class="main-header">🏃 马拉松跑鞋品牌数据分析平台</p>',
      unsafe_allow_html=True,
  )
  st.markdown(
      '<p class="sub-header">深度分析乔丹品牌及国产/国际品牌在马拉松赛场上的地位变化</p>',
      unsafe_allow_html=True,
  )

  tab1, tab2, tab3, tab4 = st.tabs(
      ["📊 总览排行", "👟 乔丹专题", "⚖️ 品牌对比", "🌏 国产vs国际"]
  )
  ...
  with tab1:
      ...
      if len(latest_data) == 0:
          ...
          ranking = (
              latest_data.groupby(["brand", "type_zh"])["share_pct"]
              .mean()
              .reset_index()
          )

          ranking = (
              ranking.sort_values("share_pct", ascending=False)
              .head(20)
      ...
      if len(jordan_data) == 0:
          ...
          with c1:
              ...
              st.metric(
                  "🏆 历史最佳排名",
                  f"第{int(best['rank'])}名",
                  f"{best['event']} {int(best['year'])}",
              )
          with c2:
              worst = jordan_data.loc[jordan_data["rank"].idxmax()]
              st.metric(
                  "📉 历史最差排名",
                  f"第{int(worst['rank'])}名",
                  f"{worst['event']} {int(worst['year'])}",
              )
          with c3:
              st.metric("📈 平均排名", f"第{jordan_data['rank'].mean():.1f}名")
          with c4:
          ...
          with col_l:
              if view_mode == "份额趋势":
                  ...
                  if aggregate_mode:
                      trend = (
                          jordan_data.groupby("year")["share_pct"]
                          .mean()
                          .reset_index()
                      )
                  else:
                      trend = (
                          jordan_data.groupby(["year", "event"])["share_pct"]
                          .mean()
                          .reset_index()
                      )
          ...
          with col_r:
              st.markdown("#### 🗺️ 各赛事表现热力图")
              heatmap_data = jordan_data.pivot_table(
                  values="rank",
                  index="event",
                  columns="year",
                  aggfunc="mean",
          ...
          if len(yearly_jordan) >= 2:
              ...
              if rank_change > 0:
                  box_class = "warning-box"
                  trend_text = (
                      f"排名从第{start_rank:.0f}名下滑至第{end_rank:.0f}名，"
                      f"下降了{rank_change:.0f}个位次"
                  )
              elif rank_change < 0:
                  box_class = "success-box"
                  trend_text = (
                      f"排名从第{start_rank:.0f}名上升至第{end_rank:.0f}名，"
                      f"提升了{abs(rank_change):.0f}个位次"
                  )
              else:
                  box_class = "insight-box"
                  trend_text = f"排名保持在第{end_rank:.0f}名左右，相对稳定"
              ...
              market_position = (
                  "乔丹在全局跑者中保持较稳定的大众化地位，但在破3精英选手中的渗透率持续下滑，"
                  "表明其在高端竞技领域的竞争力正被其他国产品牌蚕食。"
                  if cohort_filter == "破3选手"
                  else "乔丹在全局跑者市场中保持稳定份额，品牌认知度较高，但面临来自特步等头部国产品牌的激烈竞争。"
              )
              ...
              {analysis_text}<br><br>
              <strong>排名变化：</strong>{trend_text}<br><br>
              <strong>市场定位：</strong>{market_position}
              </div>
              """,
                  unsafe_allow_html=True,
              )
  ...
  with tab3:
      ...
      top_brands = (
          filtered_df.groupby("brand")["share_pct"]
          .mean()
      ...
      if len(selected_brands) < 2:
          ...
          with col_l:
              st.markdown("#### 📈 份额趋势对比")
              trend = (
                  compare_df.groupby(["year", "brand"])["share_pct"]
                  .mean()
                  .reset_index()
              )
          ...
          with col_r:
              ...
              rank_trend = (
                  ranked_compare.groupby(["year", "brand"])["rank"]
                  .mean()
                  .reset_index()
              )
          ...
          for brand in selected_brands:
              ...
              stability_std = bd["rank"].std()
              if pd.isna(stability_std):
                  stability_std = 0
              ...
              radar_data.append(
                  {
                      ...
                      "份额得分": min(100, bd["share_pct"].mean() * 5),
                      "最佳表现": max(0, min(100, 100 - bd["rank"].min() * 8)),
                      "稳定性": max(0, min(100, 100 - stability_std * 5)),
                      "赛事覆盖": bd["event"].nunique() / df["event"].nunique() * 100,
                  }
              )
  ...
  with tab4:
      ...
      type_trend = (
          filtered_df.groupby(["year", "brand_type", "type_zh"])["share"]
          .sum()
          .reset_index()
      )
      ...
      if len(type_trend) > 0:
          ...
          dom_first = type_trend[
              (type_trend["year"] == min_yr) & (type_trend["brand_type"] == "domestic")
          ]["share_pct"].sum()
          dom_last = type_trend[
              (type_trend["year"] == max_yr) & (type_trend["brand_type"] == "domestic")
          ]["share_pct"].sum()
          ...
          with c3:
              ...
              st.metric("📈 国产增长", f"{change:+.1f}%")
          with c4:
              top10_dom = filtered_df[
                  (filtered_df["rank"] <= 10) & (filtered_df["brand_type"] == "domestic")
              ]
              if len(top10_dom) > 0:
                  st.metric("🏅 TOP10国产数(均)", f"{top10_dom.groupby('year').size().mean():.1f}个")
          ...
          with col_l:
              st.markdown("#### 📊 市场份额趋势")
              yearly_type = (
                  type_trend.groupby(["year", "type_zh"])["share_pct"]
                  .sum()
                  .reset_index()
              )
          ...
          with col_r:
              st.markdown("#### 📈 TOP10品牌数量变化")
              top10_by_type = (
                  filtered_df[filtered_df["rank"] <= 10]
                  .groupby(["year", "type_zh"])
                  .size()
          ...
          with cl:
              ...
              dom_brands = ["特步", "李宁", "安踏", "鸿星尔克", "乔丹"]
              dom_trend = (
                  filtered_df[filtered_df["brand"].isin(dom_brands)]
                  .groupby(["year", "brand"])["rank"]
                  .mean()
                  .reset_index()
              )
              ...
              if len(dom_trend) > 0:
                  ...
                  fig.update_layout(
                      height=350,
                      yaxis=dict(autorange="reversed", title="排名"),
                      xaxis=dict(title="年份", dtick=1),
                  )
                  st.plotly_chart(fig, use_container_width=True)
          ...
          with cr:
              ...
              int_brands = ["Nike", "Adidas", "ASICS", "Saucony", "HOKA"]
              int_trend = (
                  filtered_df[filtered_df["brand"].isin(int_brands)]
                  .groupby(["year", "brand"])["rank"]
                  .mean()
                  .reset_index()
              )
          ...
          competition_text = "占据绝对主导地位" if dom_last > 70 else "正在快速崛起"
          challenge_text = "已打破" if dom_last > 60 else "正在挑战"
          ...
          {max_yr}年国产品牌总份额达 <strong>{dom_last:.1f}%</strong>，较{min_yr}年的{dom_first:.1f}%增长了
  <strong>{change:.1f}个百分点</strong>（增幅{growth_rate:.1f}%）。<br><br>
          <strong>2. 竞争态势</strong><br>
          """,
              unsafe_allow_html=True,
          )
  ...
  st.markdown("---")
  st.markdown(
      '<div style="text-align:center;color:#64748B;padding:1rem;">📊 马拉松跑鞋品牌分析平台 v2.0 | 数据来源：悦跑圈等平
  台</div>',
      unsafe_allow_html=True,
  )
